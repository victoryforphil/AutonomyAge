...
status: <short status>     # e.g. "PR open", "researching", "blocked on X"
last_updated: YYYY-MM-DD
---
```

`key`, `branch`, `pr`, `desc`, `status`, `last_updated` are required. Keep `desc`
and `status` to one line. Update `last_updated` on every meaningful change.
...
- Update `status`, `last_updated`, `State`, `Todos` whenever something changes.
- Do not duplicate deep specs — link to the plan/design doc.
- If a task is blocked or needs a human, say so explicitly in `status`, `State`,
  and `Human help`.
