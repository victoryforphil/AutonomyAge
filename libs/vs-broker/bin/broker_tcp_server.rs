...
use vs_data_store::topics::TopicKey;
use vs_wtf::Timespan;
#[derive(Parser)]
struct Args {
...
async fn main() {
    ...
    let (adapter_a, adapter_b) =
        vs_broker::adapters::channel::ChannelBrokerAdapter::new_pair();
    broker.add_adapter(adapter_a);

    // Create local node
    let node_info = vs_broker::node::info::BrokerNodeInfo::new("local_node");
    let mut node = vs_broker::node::BrokerNode::new(node_info, adapter_b);

    // Create printer task to monitor all topics
    let printer_task =
        vs_broker::task::example::task_printer::TaskPrinter::new(TopicKey::from_str(""));
